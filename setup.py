...
This file basically just uses CMake to compile the ossia_python python bindings project
located in the 3rdParty/libossia/OSSIA/ossia-python folder and then puts the outputs into standard
python packages.


To build the pyossia module:
...
class build(_build):
    def run(self):
        ...
        with open(os.path.join(dist_dir, '__init__.py'), 'w') as f:
            # just so that we can `import pyossia` and not `from pyossia import pyossia`
            f.write('from .pyossia import *\n')
            # add version here
...
setup(
  ...
  zip_safe=False,
  ext_modules=[Extension('ossia_python', [])],
